...
_PREDEFINED_SPLITS_PRETRAIN = {
    "sharegpt4v": ["sharegpt4v_mix665k_cap23k_coco-ap9k_lcs3k_sam9k_div2k.json"],
}

def get_metadata(name):
    if name in ['sharegpt4v']:
        return {'gt_json': os.path.join(_coco_root, 'ShareGPT4V/data/sharegpt4v/sharegpt4v_mix665k_cap23k_coco-ap9k_lcs3k_sam9k_div2k.json')}
    else:
        return {}

evaluator_mapper = {'sharegpt4v': 'sharegpt'}
...
def load_pretrain_data(arrow_root, meta, name, pretrain_arrows):
    ...
    for question in questions:
        ...
        if 'image' in question:
            ...
            if os.path.isfile(os.path.join(_coco_root, 'ShareGPT4V/data', image_id)):
                ret.append( {
                        "image_id": image_id,
...
def register_pretrain(
    ...
    semantic_name = name
    arrow_root = os.path.join(arrow_root, 'ShareGPT4V/data/sharegpt4v')
