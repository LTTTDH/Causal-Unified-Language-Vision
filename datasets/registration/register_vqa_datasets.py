...
_PREDEFINED_SPLITS_PRETRAIN = {
    "vqav2_train": ["v2_OpenEnded_mscoco_train2014_questions.json"],
    "vqav2_test": ["v2_OpenEnded_mscoco_test2015_questions.json"],
    "vqav2_test-dev": ["v2_OpenEnded_mscoco_test-dev2015_questions.json"],
    "vqav2_val": ["v2_OpenEnded_mscoco_val2014_questions.json"],
}

def get_metadata(name):
    if name in ['vqav2_train']:
        return {'gt_json': os.path.join(_coco_root, 'VQAv2/v2_mscoco_train2014_annotations.json')}
    elif name in ['vqav2_val']:
        return {'gt_json': os.path.join(_coco_root, 'VQAv2/v2_mscoco_val2014_annotations.json')}
    else:
        return {}
...
def load_pretrain_data(arrow_root, meta, name, pretrain_arrows):
    ...
    cur_id = 0
    questions = pretrain_arrows[0]['questions']
...
def register_pretrain(
    ...
    semantic_name = name
    arrow_root = os.path.join(arrow_root, 'VQAv2')
