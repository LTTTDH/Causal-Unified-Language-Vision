...
class BLIP2VQAPipeline:
    ...
    def evaluate_model(
        ...
        blip2_model = Blip2ForConditionalGeneration.from_pretrained(BLIP2_LOCAL_PATH, load_in_8bit=True, device_map=self._opt['rank'], torch_dtype=torch.bfloat16)
        ...
        for dataset_label in dataset_names:
            ...
            with torch.no_grad():
                ...
                for idx, batch in prog_bar:
                    ...
                    blip2_inputs = blip2_processor(text=prompt, images=torch.stack([b['image'] for b in batch]), return_tensors="pt")
                    
                    # Generate
                    with torch.inference_mode():
                        generate_ids = blip2_model.generate(**{k:v.cuda() for k,v in blip2_inputs.items()}, max_new_tokens=10, min_length=1, num_beams=5, length_penalty=-1)
