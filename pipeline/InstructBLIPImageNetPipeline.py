...
class InstructBLIPImageNetPipeline:
    ...
    def evaluate_model(
        ...
        if trainer.accel.is_main_process:
            import csv
            with open("problem_experiment/instructblip_imagenet.csv", "w") as f:
        ...
        for dataset_label in dataset_names:
            ...
            # accelerate wrapping
            instructblip_model, eval_batch_gen = trainer.accel.prepare(instructblip_model, eval_batch_gen)
            ...
            with torch.no_grad():
                ...
                for idx, batch in prog_bar:
                    ...
                    if idx == 0:
                        # InstructBLIP Text
                        instructblip_text_inputs = instructblip_processor(text=[f'This is {cl}' for cl in self.data_classes], images=None, padding=True, return_tensors="pt")
                        with torch.inference_mode():
                            query_outputs = instructblip_model.qformer(
                                input_ids=instructblip_text_inputs.qformer_input_ids.to(trainer.accel.device),                         # [num classes, padded len]/ [1000, 14]
                                attention_mask=instructblip_text_inputs.qformer_attention_mask.to(trainer.accel.device),      
                                return_dict=instructblip_model.config.use_return_dict,
                            )
                    ...
                    # instrcutblip inputs
                    instructblip_img_inputs = instructblip_processor(text=None, images=[x["image"] for x in batch], return_tensors="pt") # pixel_values: [batch size, C, H ,W]

                    # InstructBLIP Process
                    with torch.inference_mode():
                        vision_outputs = instructblip_model.vision_model(
                            pixel_values=instructblip_img_inputs["pixel_values"].to(trainer.accel.device),
                            return_dict=instructblip_model.config.use_return_dict,
                        )                                                                                                                # last_hidden_state, pooler_output
                        query_outputs = instructblip_model.qformer(
                            input_ids=None,
                            attention_mask=None,
                            query_embeds=instructblip_model.query_tokens.expand(vision_outputs[0].shape[0], -1, -1),                     # [batch size, 32, 768]
        ...
        for i, x in enumerate(self.evaluator):
            if n_image_list[i]==0:
                if trainer.accel.is_main_process:
                    with open("problem_experiment/instructblip_imagenet.csv", "a+", newline='') as f:
                        csv_writer = csv.writer(f)
