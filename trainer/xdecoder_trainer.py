...
class XDecoder_Trainer(DefaultTrainer):
    ...
    def create_optimizer_and_scheduler(self):
        """
        Set up self.optimizer and self.lr_scheduler

        This method initializes self.optimizer and self.lr_scheduler as dictionaries of
        instances of the classes that OPTIMIZER and LR_SCHEDULER in the config file points to.
        One optimizer and lr scheduler for each model in self.model. They have the same keys
        as self.model.
        """
        ...
        # parameters = self.model[module_name].get_training_parameters()
        # self.optimizer[module_name] = optimizer_class(parameters, **optimizer_parameters)
        # params = []
        # for module_param_name, value in self.model[module_name].named_parameters(recurse=True):
        ...
        self.lr_scheduler = build_lr_scheduler(scheduler_cfg, self.optimizer)